...
export default function Sidebar({ isOpen, onClose }) {
  return (
    <motion.div
      className={`fixed top-16 right-0 h-auto max-h-[90vh] w-64 bg-gray-900 bg-opacity-75 text-white z-50 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} backdrop-blur-lg shadow-lg`}
      initial={{ x: '100%' }}
      animate={{ x: isOpen ? '0%' : '100%' }}
      transition={{ duration: 0.3 }}
    >
      <div className="p-4 flex flex-col items-center justify-center h-full overflow-y-auto">
        <button onClick={onClose} className="text-white text-xl font-bold mb-10 hover:text-gray-400">Close</button>
        <nav>
